...
public class SimplifySumOverConstant
        implements Rule<AggregationNode>
{
    ...
    public SimplifySumOverConstant(PlannerContext plannerContext)
    {
    ...
    public Result apply(AggregationNode parent, Captures captures, Context context)
    {
        ...
        for (Entry<Symbol, AggregationNode.Aggregation> entry : parent.getAggregations().entrySet()) {
            ...
            if (isSumOverConstant(context.getSession(), aggregation, child.getAssignments())) {
                changed = true;
                Expression literalValue = aggregation.getArguments().get(0);
                if (literalValue instanceof SymbolReference) {
                    literalValue = child.getAssignments().get(Symbol.from(literalValue));
                }
                // sum(constant) can be rewritten as constant x count(*).
                Expression rewrittenSumExpression = new ArithmeticBinaryExpression(MULTIPLY, new SymbolReference("count"), literalValue);
                // TODO: Below code needs to be rewritten to make use of the rewrittenSumExpression above.
    ...
    private boolean isSumOverConstant(Session session, AggregationNode.Aggregation aggregation, Assignments inputs)
    {
        BoundSignature signature = aggregation.getResolvedFunction().getSignature();
        if (!signature.getName().equals("sum") || signature.getArgumentTypes().size() != 1) {
            return false;
        }
